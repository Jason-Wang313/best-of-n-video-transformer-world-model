# Tie-Aware Finite-Pool Selection Law

Consider a finite candidate pool with scores `S_i` and measured utilities `U_i`. A top-score video selector samples `N` candidates with replacement, takes the highest observed score, and breaks score ties uniformly among the tied sampled candidates.
...
The implementation in `src/counterfactual_video_audit/theory.py` validates this formula against brute-force enumeration for small tied pools and Monte Carlo sampling for larger checks. The same law covers oracle-aligned scores, anti-aligned scores, constant utilities, and binary success utilities.
