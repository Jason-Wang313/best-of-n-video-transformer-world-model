...
\begin{abstract}
Generated video futures are persuasive planning objects: they look like outcomes, not merely scores. This paper audits a selected-tail failure mode in which a visually smooth future is not faithful to the support assumptions that supposedly caused it. In a controlled action-conditioned RGB grid world, large candidate-count visual selection increasingly chooses videos that cross a hidden wall or emerge from an occluder without a valid action-conditioned path. The expanded suite extends raw selection to \(N=256\): visual plausibility rises by \(0.077\), action-consistency violation rises by \(0.484\), and real executed utility falls by \(1.946\); action-consistency filtering and calibrated repair each improve \(N=256\) utility by \(4.600\). We then add a CPU-bounded Moving-MNIST 10-to-10 future-prediction tier \citep{srivastava2015unsupervised}: from \(N=1\) to \(64\), raw visual selection raises internal visual score by \(0.357\) while future MSE worsens by \(0.0268\) and centroid ADE by \(4.480\); a motion-support gate reduces high-\(N\) MSE by \(0.0449\) and ADE by \(11.852\). We contribute counterfactual video auditing, a tie-aware finite-pool selection law, video-specific diagnostics, benchmark baselines, and scoped repair gates. The claim is intentionally bounded: this is a controlled warning and audit protocol for video future selection, not evidence of broad deployment validity or benchmark dominance.
\end{abstract}
...
Video prediction has long been used as an intermediate representation for physical prediction and control \citep{srivastava2015unsupervised,finn2016unsupervised,babaeizadeh2018stochastic,villegas2017learning,ebert2018visualforesight}. These systems expose a useful tension: a rollout can be visually coherent while still being wrong in the downstream quantities needed for control. Our controlled setting removes most of the modeling complexity so that the selected-tail effect can be inspected frame by frame, while the Moving-MNIST tier checks the same selection pressure on a standard bouncing-digit future-prediction protocol.
...
The original suite sweeps \(N \in \nset\). The primary failure run uses \(45\) trials per \(N\); the repair run uses \(32\) trials per \(N\) and a \(36\)-example pilot calibration set; the multi-seed check repeats the key failure and repair metrics across seeds \(120,221,322\). The expansion suite adds:
\begin{itemize}
...
\paragraph{Moving-MNIST benchmark tier.}
The v4 pass adds a CPU-bounded Moving-MNIST 10-to-10 future-prediction stress test \citep{srivastava2015unsupervised}. The generator uses \(64\times64\) frames, \(28\times28\) MNIST digits when the cache is available, deterministic digit templates for no-network smoke tests, \(48\) benchmark sequences, \(10\) context frames, \(10\) future frames, and candidate counts \(N\in\{1,4,16,64\}\). The raw selector maximizes an internal visual score based on flicker, mass preservation, sharpness, and boundary support. The comparison set includes last-frame persistence, constant-velocity bounce, a motion-support gate, and an oracle over the sampled pool. The benchmark writes compact CSV/JSON artifacts under \resultfile{results/moving_mnist_benchmark/} and a generated claim file that must pass before final audit.
...
\begin{table}[t]
  \centering
  \small
  \caption{Expanded raw candidate-count tail stress. Values are means from the expansion suite.}
  \label{tab:tail256}
...
\section{Moving-MNIST Benchmark Stress Tier}

Controlled action-conditioned videos make the causal failure inspectable, but a submission should also ask whether the selection pressure appears on a recognized video-prediction protocol. The Moving-MNIST tier answers a narrower question: if candidate videos are selected by an internal visual score, does increasing the sampled pool favor smooth-looking futures that are worse under future-frame and centroid metrics? Table~\ref{tab:moving-mnist} reports the aggregate \(48\)-sequence benchmark rows. Lower future MSE and centroid ADE are better.

\begin{center}
  \refstepcounter{table}\label{tab:moving-mnist}
  \small
  Table~\thetable: Moving-MNIST 10-to-10 benchmark stress. Raw visual selection improves the internal score while worsening future-frame and centroid errors; a motion-support gate and explicit baselines expose the failure.

  \vspace{0.4em}
  \begin{tabular}{@{}lrrrrr@{}}
    \toprule
    selector & \(N\) & visual & MSE & centroid ADE & collapse \\
    \midrule
    raw visual & 1 & 0.638 & 0.0381 & 8.649 & 0.479 \\
    raw visual & 64 & 0.995 & 0.0649 & 13.130 & 1.000 \\
    motion gate & 64 & 0.473 & 0.0199 & 1.277 & 0.000 \\
    persistence & 64 & 0.995 & 0.0649 & 13.130 & 1.000 \\
    constant-velocity bounce & 64 & 0.453 & 0.0000 & 0.000 & 0.000 \\
    oracle over sampled pool & 64 & 0.453 & 0.0000 & 0.000 & 0.000 \\
    \bottomrule
  \end{tabular}
\end{center}

The claim gates are intentionally adversarial. From \(N=1\) to \(64\), raw visual selection raises the internal visual score by \(0.357\), but future MSE worsens by \(0.0268\) and centroid ADE worsens by \(4.480\). The motion gate reduces high-\(N\) MSE by \(0.0449\) and ADE by \(11.852\) relative to raw visual selection. Persistence is a negative control for motion collapse, constant-velocity bounce is a physics-aware baseline for the synthetic bouncing-digit protocol, and the oracle row shows that better sampled futures were available even when raw visual selection chose the wrong tail. This is not a leaderboard claim: it is a benchmark stress tier showing that the visual-tail audit survives outside the hand-constructed grid world.

\begin{center}
  \includegraphics[width=0.95\linewidth]{figure10_moving_mnist_benchmark.png}
  \refstepcounter{figure}\label{fig:moving-mnist}
  \small
  Figure~\thefigure: Moving-MNIST benchmark stress. Raw visual selection climbs toward collapsed persistence as \(N\) grows, while motion-support and bounce baselines preserve the future motion metrics.
\end{center}
...
The paper claims that, in a controlled action-conditioned video world, increasing candidate count can select smoother and more goal-like videos whose attached actions execute worse. It claims that action consistency, temporal causality, occlusion uncertainty, and frame-state agreement expose the failure. It claims that simple video-specific filters and a small calibration selector repair the high-candidate tail in this scaffold. It claims that score-key choice changes high-candidate executed utility and action consistency. It also claims that a compact Moving-MNIST future-prediction tier shows the same raw visual-selection pressure against future-frame MSE and centroid ADE, under fair persistence, constant-velocity bounce, motion-gate, and oracle comparisons.

\paragraph{What is not claimed.}
The paper does not claim broad video-modeling success, real robot validation, benchmark superiority, or a universal safety guarantee. It does not claim that every visual planner fails under larger candidate counts. It does not claim that action consistency is sufficient in every environment. It does not claim that the tiny learned transformer is competitive, and the Moving-MNIST tier is not presented as a video-prediction leaderboard result.

\paragraph{Harsh reviewer attacks.}
The strongest attacks are fair. The environment is synthetic. The learned video model is smoke-scale. The repair uses simulator-accessible diagnostics. The invalid direct path is deliberately constructed. Moving-MNIST reduces the external-validity gap but remains a simple synthetic benchmark. The finite-pool law describes sampled candidate pools, not every correlated beam-search procedure. The paper survives these attacks only if it stays bounded. Its contribution is a reproducible counterfactual video audit and a precise failure pattern, not a field-level benchmark result.

\section{Limitations}

This paper is deliberately scoped. The environment is synthetic, low resolution, deterministic, and designed to make action inconsistency inspectable. The Moving-MNIST tier adds a recognized future-prediction stress protocol but remains small and CPU-bounded. The learned transformer is tiny and CPU-trained. The repair ladder uses diagnostics available in this environment and would need new calibration labels before being used elsewhere. We do not evaluate physical robots, leaderboard-scale video benchmarks, foundation video predictors, or arbitrary world-model planners. The supported claim is narrower: in controlled video-future selection settings, large candidate-count visual selection can move probability mass toward plausible but unsupported futures, and counterfactual execution or future-metric auditing plus finite-pool accounting makes that failure measurable.

The main missing evidence is neural validation at scale. A stronger paper would train larger action-conditioned video models, evaluate on richer environments, include standard video-prediction leaderboards, and test whether action-consistency diagnostics predict real execution failures when videos are not generated by an inspectable simulator. The present paper is a mechanism study and benchmark scaffold for that next step.

\section{Conclusion}

Video futures can be seductive planning objects because they look like outcomes. This paper shows a controlled case where that visual concreteness is misleading: larger candidate-count selection chooses smoother and more goal-like videos while the attached actions execute worse. The key audit move is counterfactual pairing. For every selected generated future, execute the same actions under the real dynamics and compare the result. The expanded evidence shows the tail through \(N=256\), repairs it with video-specific consistency checks, records score-key, horizon, and occlusion stress tests, and adds a Moving-MNIST benchmark tier where raw visual selection worsens future prediction metrics under fair baselines. The paper is bounded, but it is complete as a submission-ready mechanism study: it gives a visible failure, an exact selection law, diagnostics, repairs, negative controls, benchmark stress, and a reproducible claim audit.
...
The v4 claim audit passes the following generated expansion checks:
\begin{itemize}
...
\end{itemize}
The Moving-MNIST benchmark tier adds the following generated checks:
\begin{itemize}
  \item Raw visual selection from \(N=1\) to \(64\) raises the internal visual score by \(0.357\), above the \(0.100\) threshold.
  \item The same raw selection worsens future-frame MSE by \(0.0268\), above the \(0.004\) threshold.
  \item The same raw selection worsens centroid ADE by \(4.480\), above the \(2.000\) threshold.
  \item The motion-support gate reduces high-\(N\) future MSE by \(0.0449\), above the \(0.005\) threshold.
  \item The motion-support gate reduces high-\(N\) centroid ADE by \(11.852\), above the \(3.000\) threshold.
  \item The constant-velocity bounce baseline beats persistence on future MSE by \(0.0649\), above the \(0.008\) threshold.
  \item The oracle gap is \(0.0649\), above the \(0.008\) threshold, showing that better futures were present in the sampled pool.
\end{itemize}
...
The first tier should use controlled grid and maze videos with known action constraints. The second tier should use standard future-prediction benchmarks and richer pixel-control environments with occlusions and contact dynamics. The third tier should use held-out execution in a simulator or robot-like benchmark. The current paper covers a controlled action tier plus a compact Moving-MNIST future-prediction tier; it still lacks richer action-conditioned simulators and hardware.
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\linewidth}{@{}Xrrr@{}}
    ...
    Moving-MNIST visual-score gain & 0.357 & \(>0.100\) & yes \\
    Moving-MNIST MSE worsening & 0.0268 & \(>0.004\) & yes \\
    Moving-MNIST centroid-ADE worsening & 4.480 & \(>2.000\) & yes \\
    Moving-MNIST motion-gate MSE repair & 0.0449 & \(>0.005\) & yes \\
    Moving-MNIST motion-gate ADE repair & 11.852 & \(>3.000\) & yes \\
    Moving-MNIST bounce-over-persistence MSE gap & 0.0649 & \(>0.008\) & yes \\
    Moving-MNIST oracle MSE gap & 0.0649 & \(>0.008\) & yes \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\linewidth}{@{}p{0.23\linewidth}p{0.25\linewidth}X@{}}
    ...
    limits & broad model exploitation caveat & no leaderboard claim about modern video benchmarks or physical robot deployment \\
    \bottomrule
  \end{tabularx}
\end{table}
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\linewidth}{@{}p{0.30\linewidth}X@{}}
    ...
    \midrule
    The environment is synthetic. & Correct. The contribution is a controlled mechanism and audit scaffold, plus a compact Moving-MNIST benchmark stress tier, not broad external validity. \\
    The failure is hand-constructed. & The trap families are constructed to isolate the mechanism; stress tests show how candidate count, horizon, occlusion, score key, benchmark setting, and repair change the mechanism. \\
    ...
    Visual plausibility is not a real learned reward. & The paper includes a learned-video proxy and a Moving-MNIST benchmark tier, while keeping transparent scores so selected-tail failure is auditable. \\
...
\end{enumerate}
The current results support the first step and a compact benchmark tier. The staged plan is included to clarify what the paper does not yet prove.
...
\begin{table}[tbp]
  ...
  \begin{tabularx}{\linewidth}{@{}p{0.25\linewidth}p{0.26\linewidth}X@{}}
    ...
    expansion suite & expansion claims JSON & Candidate-count, horizon, occlusion, score-key, and repair checks pass thresholds. \\
    Moving-MNIST benchmark & benchmark claims JSON & Raw visual selection worsens future metrics, motion support repairs, and baselines/oracle are reported. \\
...
The following self-attack log records reviewer-style objections used while hardening the paper. The point is to attack the claim from multiple angles before submission and leave only claims that the evidence can carry.

\begin{table}[tbp]
  \centering
  \small
  \caption{Self-attack log for the video paper.}
  \label{tab:self-attack-log}
  \begin{tabularx}{\linewidth}{@{}p{0.30\linewidth}X@{}}
    \toprule
    attack angle & response \\
    \midrule
    ...
    The paper has no recognized benchmark. & The v4 pass adds a Moving-MNIST future-prediction tier with persistence, constant-velocity bounce, motion-gate, and oracle baselines, while avoiding leaderboard claims. \\
...
\begin{center}
  ...
  \begin{tabularx}{\linewidth}{@{}p{0.24\linewidth}p{0.20\linewidth}X@{}}
    ...
    Benchmark stress & Pass & Moving-MNIST future-prediction selection is tested with persistence, bounce, motion-gate, and oracle baselines. \\
